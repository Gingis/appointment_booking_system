  ...
  template: `
    <div class="max-w-2xl mx-auto px-4 py-10">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
        ...
        <div class="flex items-center gap-5">
          ...
          <div>
            ...
            <button (click)="fileInput.click()" class="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-700 transition-colors">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
        ...
        @if (profileSuccess()) { <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-2.5 rounded-xl text-sm mb-4">Profile updated successfully!</div> }
        @if (profileError()) { <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-2.5 rounded-xl text-sm mb-4">{{ profileError() }}</div> }
        <form [formGroup]="profileForm" (ngSubmit)="onUpdateProfile()">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input [value]="auth.user()?.email" type="email" disabled class="w-full px-4 py-2.5 border border-slate-100 bg-slate-50 rounded-xl text-slate-400 cursor-not-allowed">
            </div>
          </div>
          <button type="submit" [disabled]="savingProfile()" class="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-blue-700 disabled:opacity-60 transition-colors">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        @if (pwSuccess()) { <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-2.5 rounded-xl text-sm mb-4">Password changed successfully!</div> }
        @if (pwError()) { <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-2.5 rounded-xl text-sm mb-4">{{ pwError() }}</div> }
        <form [formGroup]="pwForm" (ngSubmit)="onChangePassword()">
          <div class="space-y-4 mb-4">
            <div>
              ...
              <input formControlName="currentPassword" type="password" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1.5">New Password</label>
              <input formControlName="newPassword" type="password" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
          </div>
          <button type="submit" [disabled]="savingPw()" class="bg-slate-800 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-slate-900 disabled:opacity-60 transition-colors">
...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (u) this.profileForm.patchValue({ name: u.name, phone: u.phone || '', studentId: u.studentId || '', course: u.course || '', yearLevel: u.yearLevel || '' });
  }
